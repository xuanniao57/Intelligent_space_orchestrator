...
> G1 的历史场景层工具（安全检查、递冰水等）仅保留为旧 demo / dry-run 语义。当前主路径是机器人 Ubuntu 主机统一网关 `192.168.1.172:8731`：中控生成统一 JSON，网关再调用本机动作、语音、导航、视频、音频、ASR 程序。完整接口见 `docs/ROBOT_HOST_UNIFIED_INTERFACE.md`。

| 功能ID | 功能 | 操作 IP:PORT | 命令 | 格式 | 指令类型 | 含义 |
|---|---|---|---|---|---|---|
| `g1_safety_check` | 安全预检查 | 192.168.1.172:8731 /api/g1/execute; PC2 decodes JSON and calls Unitree SDK | `{"primitive":"unitree_sdk_call","source_primitive":"safety_check","layer":"bridge","client":"SafetyGuard","method":"CheckPreconditions","args":{"dry_run":true,"min_human_distance_m":0.8,"speed_limit_mps":0.25}}` | JSON DeviceCommand | `g1.unitree_sdk_sequence.step` | 检查调试模式、人距、限速和急停状态。 |
| `g1_speak_notice` | 语音播报意图 | 192.168.1.172:8731 /api/g1/execute; PC2 decodes JSON and calls Unitree SDK | `{"primitive":"unitree_sdk_call","source_primitive":"speak","layer":"onboard_io","client":"AudioClient","method":"TtsMaker","args":{"text_cn":"检测到热感上升，我将启动清凉联动并递送冰水。","speaker_id":0}}` | JSON DeviceCommand | `g1.unitree_sdk_sequence.step` | G1 在动作前播报当前行动意图。 |
| `g1_move_probe` | 短程动作探针 | 192.168.1.172:8731 /api/g1/execute; PC2 decodes JSON and calls Unitree SDK | `{"primitive":"unitree_sdk_call","source_primitive":"move_probe","layer":"unitree_high_level","client":"LocoClient","method":"SetVelocity","args":{"vx":0.15,"vy":0,"omega":0,"duration":1.2,"dry_run_only":true}}` | JSON DeviceCommand | `g1.unitree_sdk_sequence.step` | 用于校园网和 SDK 桥接测试的短程 dry-run 动作。 |
| `g1_navigate_water_station` | 导航至冰水点 | 192.168.1.172:8731 /api/g1/execute; PC2 decodes JSON and calls Unitree SDK | `{"primitive":"unitree_sdk_call","source_primitive":"navigate","layer":"bridge","client":"WaypointPlanner","method":"PlanThenLocoSetVelocity","args":{"waypoint":"ice_water_station","output_client":"LocoClient","output_method":"SetVelocity","dry_run_only":true}}` | JSON DeviceCommand | `g1.unitree_sdk_sequence.step` | 向预设冰水补给点移动。 |
| `g1_deliver_ice_water` | 递送冰水 | 192.168.1.172:8731 /api/g1/execute; PC2 decodes JSON and calls Unitree SDK | `{"primitive":"unitree_sdk_call","source_primitive":"deliver_ice_water","layer":"station_tool","client":"WaterStationAdapter","method":"DeliverItem","args":{"item":"iced_bottled_water","handoff_zone":"cooling_handoff_01","dry_run_only":true}}` | JSON DeviceCommand | `g1.unitree_sdk_sequence.step` | 触发冰水补给适配器与递送意图。 |
| `g1_report_ready` | 回报完成 | 192.168.1.172:8731 /api/g1/execute; PC2 decodes JSON and calls Unitree SDK | `{"primitive":"unitree_sdk_call","source_primitive":"report_ready","layer":"feedback","client":"FeedbackAdapter","method":"ReportReady","args":{"status":"ready"}}` | JSON DeviceCommand | `g1.unitree_sdk_sequence.step` | 向中枢 ACK 回路报告动作完成。 |
| `g1_heat_delivery_sequence` | 热感冰水递送链 | 192.168.1.172:8731 /api/g1/execute; PC2 decodes JSON and calls Unitree SDK | `{"task_id":"manual_heat_delivery","scene_id":"heat_cooling_loop","speech_cn":"清凉补给已送达。","safety":{"dry_run":true,"speed_limit_mps":0.25,"min_human_distance_m":0.8},"sdk_sequence":[{"primitive":"unitree_sdk_call","source_primitive":"safety_check","layer":"bridge","client":"SafetyGuard","method":"CheckPreconditions","args":{"dry_run":true,"min_human_distance_m":0.8}},{"primitive":"unitree_sdk_call","source_primitive":"speak","layer":"onboard_io","client":"AudioClient","method":"TtsMaker","args":{"text_cn":"检测到热感上升，我将递送冰水。","speaker_id":0}},{"primitive":"unitree_sdk_call","source_primitive":"deliver_ice_water","layer":"station_tool","client":"WaterStationAdapter","method":"DeliverItem","args":{"item":"iced_bottled_water","dry_run_only":true}},{"primitive":"unitree_sdk_call","source_primitive":"report_ready","layer":"feedback","client":"FeedbackAdapter","method":"ReportReady","args":{"status":"ready"}}]}` | JSON DeviceCommand | `g1.unitree_sdk_sequence` | 清凉联动的完整 dry-run 动作链。 |
...
| `speaker_play_cocktail_01` | 中控本机播放 1 号音乐 | 192.168.1.50:22001 /api/command | `{"task_id":"manual_audio_01","op":"play","content_id":"audio_01_music_cocktail_loop","slot":1,"volume":0.62,"loop":true}` | JSON DeviceCommand | `speaker.play` | 发送到中控本机播放器/音响网关，播放音乐鸡尾酒循环音频。 |
| `speaker_play_cooling_notice_02` | 中控本机播放 2 号提示音 | 192.168.1.50:22001 /api/command | `{"task_id":"manual_audio_02","op":"play","content_id":"audio_02_cooling_notice","slot":2,"volume":0.58,"loop":false}` | JSON DeviceCommand | `speaker.play` | 发送到中控本机播放器/音响网关，播放清凉联动提示音。 |
| `speaker_stop` | 停止中控本机音响 | 192.168.1.50:22001 /api/command | `{"task_id":"manual_speaker_stop","op":"stop"}` | JSON DeviceCommand | `speaker.stop` | 停止中控本机播放器/音响网关输出。 |
...
|---|---|---|---|---|---|---|
| `0` | release arm | `192.168.1.50:8798 /api/g1/actions/execute` | `G1ArmActionClient.ExecuteAction(action_map["release arm"])` | JSON -> central direct Unitree SDK2 call | `G1ArmActionClient.ExecuteAction` | 释放手臂/回到手臂释放状态 |
| `1` | shake hand | `192.168.1.50:8798 /api/g1/actions/execute` | `ExecuteAction(action_map["shake hand"]); sleep(2); release arm` | JSON -> central direct Unitree SDK2 call | `G1ArmActionClient.ExecuteAction` | 握手后释放手臂 |
| `2` | high five | `192.168.1.50:8798 /api/g1/actions/execute` | `ExecuteAction(action_map["high five"]); sleep(2); release arm` | JSON -> central direct Unitree SDK2 call | `G1ArmActionClient.ExecuteAction` | 击掌后释放手臂 |
| `3` | hug | `192.168.1.50:8798 /api/g1/actions/execute` | `ExecuteAction(action_map["hug"]); sleep(2); release arm` | JSON -> central direct Unitree SDK2 call | `G1ArmActionClient.ExecuteAction` | 拥抱动作后释放手臂 |
| `4` | high wave | `192.168.1.50:8798 /api/g1/actions/execute` | `ExecuteAction(action_map["high wave"])` | JSON -> central direct Unitree SDK2 call | `G1ArmActionClient.ExecuteAction` | 高位挥手 |
| `5` | clap | `192.168.1.50:8798 /api/g1/actions/execute` | `ExecuteAction(action_map["clap"])` | JSON -> central direct Unitree SDK2 call | `G1ArmActionClient.ExecuteAction` | 鼓掌 |
| `6` | face wave | `192.168.1.50:8798 /api/g1/actions/execute` | `ExecuteAction(action_map["face wave"])` | JSON -> central direct Unitree SDK2 call | `G1ArmActionClient.ExecuteAction` | 面前挥手 |
| `7` | left kiss | `192.168.1.50:8798 /api/g1/actions/execute` | `ExecuteAction(action_map["left kiss"])` | JSON -> central direct Unitree SDK2 call | `G1ArmActionClient.ExecuteAction` | 左手飞吻 |
| `8` | heart | `192.168.1.50:8798 /api/g1/actions/execute` | `ExecuteAction(action_map["heart"]); sleep(2); release arm` | JSON -> central direct Unitree SDK2 call | `G1ArmActionClient.ExecuteAction` | 比心后释放手臂 |
| `9` | right heart | `192.168.1.50:8798 /api/g1/actions/execute` | `ExecuteAction(action_map["right heart"]); sleep(2); release arm` | JSON -> central direct Unitree SDK2 call | `G1ArmActionClient.ExecuteAction` | 右手比心后释放手臂 |
| `10` | hands up | `192.168.1.50:8798 /api/g1/actions/execute` | `ExecuteAction(action_map["hands up"]); sleep(2); release arm` | JSON -> central direct Unitree SDK2 call | `G1ArmActionClient.ExecuteAction` | 双手举起后释放手臂 |
| `11` | x-ray | `192.168.1.50:8798 /api/g1/actions/execute` | `ExecuteAction(action_map["x-ray"]); sleep(2); release arm` | JSON -> central direct Unitree SDK2 call | `G1ArmActionClient.ExecuteAction` | x-ray 动作后释放手臂 |
| `12` | right hand up | `192.168.1.50:8798 /api/g1/actions/execute` | `ExecuteAction(action_map["right hand up"]); sleep(2); release arm` | JSON -> central direct Unitree SDK2 call | `G1ArmActionClient.ExecuteAction` | 右手举起后释放手臂 |
| `13` | reject | `192.168.1.50:8798 /api/g1/actions/execute` | `ExecuteAction(action_map["reject"]); sleep(2); release arm` | JSON -> central direct Unitree SDK2 call | `G1ArmActionClient.ExecuteAction` | 拒绝动作后释放手臂 |
| `14` | right kiss | `192.168.1.50:8798 /api/g1/actions/execute` | `ExecuteAction(action_map["right kiss"])` | JSON -> central direct Unitree SDK2 call | `G1ArmActionClient.ExecuteAction` | 右手飞吻 |
| `15` | two-hand kiss | `192.168.1.50:8798 /api/g1/actions/execute` | `ExecuteAction(action_map["two-hand kiss"])` | JSON -> central direct Unitree SDK2 call | `G1ArmActionClient.ExecuteAction` | 双手飞吻 |
