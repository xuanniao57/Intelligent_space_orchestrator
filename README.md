...
建议所有正式变更都通过分支和 Pull Request 合并到 `main`。普通贡献者从最新 `main` 创建功能分支或 fork，提交 PR 后由维护者审核；接口、设备命令、Agent runtime、语义注册表等高风险变更需要更严格审核。
...
- 贡献者确认自己有权提交相关代码、文档、数据和素材。
- 贡献被合并后，项目维护者可以在本项目范围内使用、修改、发布和维护这些内容。
- 不提交 `.env`、API key、设备 token、真实人员隐私数据、未经脱敏的现场日志或来源不明的第三方素材。
- 涉及真实机器人、喷雾、音响、投影等现场设备的改动，必须先用无硬件模式、Fake G1 或模拟 payload 验证，并说明现场安全边界。
- 分歧无法解决时，由仓库所有者结合项目目标、现场安全和维护成本做最终决定。

### 上传版本号标注

每次上传分支或提交 Pull Request 时，贡献者需要在 PR 标题、提交说明或变更描述里标注一个“贡献版本号”。贡献版本号用于追踪协作上传记录，不等同于正式 release tag。
...
contrib-v0.4.1-20260629-frontend-lihua-02
```

标注规则：

- `目标项目版本` 使用当前准备合入的项目版本号，例如 `0.4.1`。不确定时沿用 README 或最新 release 中的版本号。
- `YYYYMMDD` 使用上传当天日期。
- 同一天同一贡献者多次上传时，序号从 `01`、`02` 递增。
- 正式发布版本由维护者统一打 Git tag，例如 `v0.4.1`、`v0.5.0`。
